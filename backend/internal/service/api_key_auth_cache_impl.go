...
const apiKeyAuthSnapshotVersion = 20 // v20: TransitAI auth, profit control, and upstream search/audio/video pricing fields
...
func (s *APIKeyService) snapshotToAPIKey(key string, snapshot *APIKeyAuthSnapshot) *APIKey {
	...
	apiKey := &APIKey{
		...
		Key:         key,
		Name:        snapshot.Name,
		Source:      normalizeAPIKeySource(snapshot.Source),
